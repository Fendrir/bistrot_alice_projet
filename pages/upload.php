...
if(htmlspecialchars(!isset($nickSession), ENT_QUOTES) || htmlspecialchars($nickSession, ENT_QUOTES) !== 'Franck') {       //Si le user est arrivé ici par "hasard"
    ...
    // On se sert du GET pour sélectionner la ligne dans la DB et la modifier
    ...
    //Si FILES['image']['error'] > 0, alors il y a eu un problème, on retourne une erreur
    if (htmlspecialchars($_FILES['image']['error'], ENT_QUOTES) > 0) {        //S'il y a eu des erreurs lors de l'upload
        ...
        //Stockage des extensions valides. Ce sont des images, on limite donc à jpg, jpeg, gif et png
        ...
        if(in_array($extension_upload, $extensions_valides)) {     //Si l'extension du fichier correspond aux formats jpg, jpeg, gif ou png
            ...
            //Si le dossier images n'existe pas on le créer
            if (!is_dir('images')) {
                mkdir('images');
            }
            ...
            if ($resultat) {                                                                                  //Si tout est OK, mise à jour de la DB
                $titre = htmlspecialchars($_POST['titre'], ENT_QUOTES);                                 //Récupération du titre input
                $sql2 = "UPDATE carte SET car_title = '$titre', car_img = '$pathDB' WHERE car_oid = $id";     // Update de la DB
                $conn->query($sql2);
                ?>
                <!-- Résumé du post -->
                <div class="col text-center">
